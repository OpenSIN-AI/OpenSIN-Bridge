...
async function ensure() {
  if (!chrome.offscreen) {
    throw new BridgeError(ERROR_CODES.UNSUPPORTED, 'chrome.offscreen API not available');
  }
...
export async function request(payload, timeoutMs = 10_000) {
  ...
  if (resp?.error) {
    throw new BridgeError(resp.code || ERROR_CODES.OFFSCREEN_ERROR, resp.error);
  }
  return resp;
}
